...
public class SRT_Scheduling extends RootScheduling {
    @Override
    public void scheduling() {
        ...
        int presntTime = processes.get(0).getProcessArriveTime();
        // 프로세스의 시간할당량을 설정함.
        ...
        while (!processes.isEmpty()) {
            ...
            for (ProcessInfo process : processes) {
            	if (process.getProcessArriveTime() <= presntTime) {
            ...
            this.getGanttChartLists().add(new GanttChart(process.getProcessPid(), presntTime, presntTime + oneBurstTime, process.getProcessColor()));
            // 다음 프로세스의 시작시간을 현재 시간에 한번에 실행하는 시간 단위를 더한 값으로 설정한다.
            presntTime += oneBurstTime;
            // 이번 타임 슬라이스에 실행한 프로세스를 삭제한다.(순서에 맞게 다시 추가할 예정)
            ...
            if (process.getProcessBurstTime() > processTimeQuantum) {
                ...
                for (int i = 0; i < processes.size(); i++) {
                    ...
                    if (processes.get(i).getProcessArriveTime() > presntTime) {
            ...
            // 만약 남은 실행시간이 시간할당량보다 적거나 같으면 프로세스의 남은 실행시간을 0으로 만든다.
